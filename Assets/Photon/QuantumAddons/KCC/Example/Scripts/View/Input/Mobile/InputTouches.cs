namespace Quantum
{
	...
	public sealed partial class InputTouches
	{
		...
		public void Update()
		{
			...
			for (int i = 0, count = UnityEngine.Input.touches.Length; i < count; ++i)
			{
				TouchControl touchControl = UnityEngine.Input.touches[i];

				InputTouch activeTouch = GetActiveTouch(touchControl.fingerId);
				if (activeTouch == null)
				{
					if (touchControl.pressure <= 0.0f)
						continue;
