namespace Quantum
{
	...
	using TouchControl = UnityEngine.Touch;
	...
	public sealed class InputTouch
	{
		...
		public void Update(TouchControl touchControl)
		{
			...
			TouchPhase phase = touchControl.phase;
			if (_current.State == ETouchState.Paused && phase != TouchPhase.Ended && phase != TouchPhase.Canceled)
				return;
			...
			if (_delta.Position.Equals(Vector2.zero))
			{
